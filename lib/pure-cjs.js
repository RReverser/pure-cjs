var fs = require('fs'),
	...
	defineLazy = function (object, propName, propGetter) {
		return Object.defineProperty(object, propName, {
			enumerable: true,
			configurable: true,
			get: function () {
				var value = propGetter.call(this);
	...
	Replacer = recast.Visitor.extend({
		...
		init: function (modulePath, parent) {
			...
			this.id = this.getId('');
			defineLazy(this, 'promise', function () {
		...
		getId: function (path) {
			...
			var self = this,
				promise = whenReadFile(path, {encoding: 'utf-8'}).then(function (js) {
					...
					replacer.visit(ast);
					return replacer.promise.then(function () {
			...
			return ids[path] = this.modulesArray.push(promise) - 1;
		},
...
exports.transform = function (inOptions) {
	...
	function normalizePathSep(path) {
		return path.replace(badPathSep, '/');
	}
	...
	var options = {
		input: normalizePathSep(toValue(inOptions.input))
	};
	
	options.output = normalizePathSep(toValue(
	...
	return replacer.promise.then(function () {
		...
		stmts.push(
			b.expressionStatement(b.assignmentExpression(
				'=',
				astLocalReqModules,
				b.arrayExpression(replacer.modulesArray.map(function (promise) {
