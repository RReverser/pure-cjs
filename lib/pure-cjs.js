...
exports.transform = function (options) {
	...
	options.input = options.input.replace(badPathSep, '/');

	options.output = options.output ? options.output.replace(badPathSep, '/') : options.input.replace(jsExtRegExp, '.out.js');
	...
	return replacer.promise.then(function () {
		...
		return whenWriteFile(options.output, recast.print(b.program(stmts)));
	});
};